...
export default function BookTalks({ bookId }) {
	...
	async function renderExistingComments() {
		...
		try {
			...
			const response = await fetch(
				`${baseUrl}/comments?itemId=${bookId}`,
				{
					headers: {
			...
			const uniqueUserIds = [
				...new Set(data.comments.map((c) => c.user)),
			];
			const userMap = {};
			for (const uid of uniqueUserIds) {
				...
				try {
					const userResp = await fetch(
						`${baseUrl}/auth/profile/public?id=${uid}`,
					);
					if (!userResp.ok) {
						console.error(
							`[BookTalks] GET user ${uid} =>`,
							userResp.status,
						);
					...
					console.error(
						`[BookTalks] error fetching user ${uid}:`,
						err,
					);
					userMap[uid] = { name: "Anonymous", photo: "" };
				}
	...
	async function addNewComment(newReview) {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to create comment. Status: ${response.status}`,
				);
			}
	...
	async function likeComment(commentId) {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to like/unlike comment. Status: ${response.status}`,
				);
			}
